...
```{r}
# Loading the Dataset
df<- read.csv("/Users/kat/Documents/SeattleU/Spring 23/Written Homework-4/spotify.csv", header = TRUE, sep = ",")
```
...
imp_weight <- pr.out$rotation[, 1]
#Fetch the 7 important variables based on their weights
sorted_weights <- sort(abs(imp_weight), decreasing = TRUE)
important_variables <- names(sorted_weights)[1:7]
important_variables
```
...
```{r}
#Plot the dendrogram with complete linkage with the number of clusters as 3
plot(hc.complete, hang = -1,main = "Hierarchical Clustering Dendrogram", xlab = "", sub = "", cex = .4)
rect.hclust(hc.complete, 3, border = 3)
```
